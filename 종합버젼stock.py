...
def pipe_items(text: str, limit: int | None = None) -> list[str]:
    items = [item.strip() for item in str(text or "-").split("|") if item.strip()]
    return items[:limit] if limit else items


def names_from_candidates(text: str, limit: int = 8) -> str:
    names: list[str] = []
    for item in pipe_items(text):
        name = item.split("(", 1)[0].strip()
        if name:
            names.append(name)
    return compact_list(names, limit)


def detail_lines(text: str, limit: int = 4) -> list[str]:
    return [f"  - {item}" for item in pipe_items(text, limit)]


def sector_summary(text: str, limit: int = 6) -> str:
    items = []
    for item in pipe_items(text, limit):
        name = item.split(":", 1)[0].strip()
        value = item.split(":", 1)[1].strip() if ":" in item else ""
        items.append(f"{name} {value}".strip())
...
def overlap_items(left: list[str], right: list[str], fallback: list[str], limit: int = 3) -> list[str]:
    right_set = set(right)
    overlap = [item for item in left if item in right_set]
    result = overlap or fallback
    return [item for item in result if item and item != "-"][:limit]


def stock_members_for_sector(sector: str) -> list[tuple[str, str]]:
    names = STOCK_SECTOR_ALIASES.get(sector, [sector])
    members: list[tuple[str, str]] = []
    seen: set[str] = set()
    for name in names:
        for stock_name, ticker in stocktrend_full_v3.SECTORS.get(name, []):
            if ticker not in seen:
                members.append((stock_name, ticker))
                seen.add(ticker)
    return members


def top_stock_lines_by_sector(sectors: list[str], target_day: date) -> list[str]:
    sector_members = {sector: stock_members_for_sector(sector) for sector in sectors}
    tickers = sorted({ticker for members in sector_members.values() for _name, ticker in members})
    if not tickers:
        return ["- 실제 종목 데이터 없음"]

    raw = yf.download(tickers, period="9mo", auto_adjust=True, progress=False, threads=True)
    if isinstance(raw.columns, pd.MultiIndex):
        close = raw["Close"].sort_index().dropna(how="all")
        volume = raw["Volume"].sort_index().dropna(how="all") if "Volume" in raw.columns.get_level_values(0) else pd.DataFrame(index=close.index)
    else:
        close = raw[["Close"]].rename(columns={"Close": tickers[0]})
        volume = raw[["Volume"]].rename(columns={"Volume": tickers[0]})

    day = nearest(close.index, pd.Timestamp(target_day))
    if day is None:
        return ["- 실제 종목 데이터 없음"]
    ...
    for sector in sectors:
        scored = []
        for stock_name, ticker in sector_members.get(sector, []):
            ...
            if pos < 20:
                continue
            ret5 = series.pct_change(5).iloc[pos] * 100
            ret20 = series.pct_change(20).iloc[pos] * 100
            vol_ratio = np.nan
            if not volume.empty and ticker in volume.columns:
                vol_series = volume[ticker].dropna()
                if day in vol_series.index:
                    vol_pos = vol_series.index.get_loc(day)
                    avg20 = vol_series.rolling(20).mean().iloc[vol_pos]
                    vol_ratio = vol_series.iloc[vol_pos] / avg20 if avg20 else np.nan
            score = float(np.nan_to_num(ret5, nan=0)) * 2 + float(np.nan_to_num(ret20, nan=0)) + float(np.nan_to_num(vol_ratio, nan=1)) * 5
            scored.append((stock_name, score, ret5, ret20, vol_ratio))
        scored.sort(key=lambda item: item[1], reverse=True)
        top = scored[:3]
        if not top:
            lines.append(f"- {sector}: 실제 종목 데이터 없음")
            continue
        stock_text = ", ".join(f"{name}(5D {ret5:+.1f}%)" for name, _score, ret5, _ret20, _vol in top)
        lines.append(f"- {sector}: {stock_text}")
    return lines
...
def build_signal_section(start: date, end: date) -> tuple[str, list[str]]:
    ...
    if rows:
        latest = rows[-1]
        risks = " -> ".join(r.get("Signal_Count", "-") for r in rows)
        buys = " -> ".join(r.get("Buy_Count", "-") for r in rows)
        lines = [
            section_header("🧭 [01]", "SIGNAL", "리스크 / 포지션 / 포트"),
            "📌 현재 판단",
            bullet(f"{latest.get('Mode', '-').replace('_MODE', '')} | {latest.get('Position_Level', '-')}"),
            bullet(f"주식 {latest.get('Recommended_Stock_Position', '-')} / 현금 {latest.get('Recommended_Cash_Position', '-')}"),
            bullet(f"위험신호 {latest.get('Signal_Count', '-')} / 매수후보 {latest.get('Buy_Count', '-')}"),
            "",
            "📆 주간 흐름",
        ]
        ...
        lines.extend(
            [
                "",
                "🧩 포트 변화",
                bullet(f"섹터: {sector_summary(latest.get('Portfolio_Sectors', '-'), 6)}"),
                bullet(f"후보: {names_from_candidates(latest.get('Portfolio_Candidates', '-'), 5)}"),
                bullet(f"교체: {names_from_candidates(latest.get('Portfolio_Replacement', '-'), 6)}"),
            ]
        )
        watch = [
    ...
    lines = [
        section_header("🧭 [01]", "SIGNAL", "리스크 / 포지션 / 포트"),
        "📌 현재 판단",
        bullet(f"{latest.get('Mode', '-')} | {latest.get('Position_Level', '-')}"),
        bullet(f"주식 {latest.get('Recommended_Stock_Position', '-')} / 현금 {latest.get('Recommended_Cash_Position', '-')}"),
        bullet(f"위험신호 {latest.get('Signal_Count', '-')}"),
        "",
        "🧩 포트 변화",
        bullet(f"섹터: {sector_summary(latest.get('Portfolio_Sectors', '-'), 6)}"),
        bullet(f"후보: {names_from_candidates(latest.get('Portfolio_Candidates', '-'), 5)}"),
...
def build_etf_rs_section(start: date, end: date) -> tuple[str, list[str]]:
    ...
    lines = [
        section_header("📈 [02]", "ETF RS", "상대강도 / 순위변화"),
        "📆 월~금 TOP3",
        *daily_top3,
        "",
        "🏁 금요일 TOP5",
...
def build_stocktrend_section(start: date, end: date) -> tuple[str, list[str]]:
    ...
    lines = [
        section_header("🔥 [03]", "주식트렌드", "강관심 섹터 / 실제 종목"),
        "📆 월~금 TOP3",
        *daily_top3,
        "",
        "🏁 금요일 섹터 TOP5",
        *friday_rows,
        "",
        "📌 실제 종목",
        *top_stock_lines_by_sector(final_names[:3], end),
    ]
    return "\n".join(lines), final_names
...
def build_etf_ma_section(start: date, end: date) -> tuple[str, list[str]]:
    ...
    lines = [
        section_header("📊 [04]", "ETF 20일선", "이평 / 정배열 / 관심후보"),
        "📆 월~금 MA TOP3",
        *daily_top3,
        "",
        "🏁 금요일 TOP5",
...
def build_message() -> str:
    ...
    rs_ma_candidates = overlap_items(rs_watch, ma_watch, rs_watch + ma_watch, 4)
    signal_candidate_text = extract_after(signal_text, "후보:")
    replace_candidate_text = extract_after(signal_text, "교체:")
    sector_candidate_text = compact_list(stock_watch, 3)
    ma_candidate_text = compact_list(ma_watch, 4)
    rs_candidate_text = compact_list(rs_watch, 4)
    summary = [
        section_header("🎯 [05]", "종합 판단", "다음 주 전략"),
        "핵심 체크",
        ...
        "",
        "다음 주 전략",
        bullet(f"기본: {extract_after(signal_text, '주식 ')} 기준, 공격 포지션 유지"),
        bullet(f"1순위: RS+이평 동시 후보 - {compact_list(rs_ma_candidates, 4)}"),
        bullet(f"2순위: 강관심 섹터 - {sector_candidate_text}"),
        bullet(f"진입: RS 후보({rs_candidate_text})와 MA 후보({ma_candidate_text}) 겹침 확인"),
        bullet(f"포트 후보: {signal_candidate_text}"),
        bullet(f"교체 후보: {replace_candidate_text}"),
        bullet("리스크: 위험신호 2개 이상이면 신규매수 축소"),
        "",
        "다음 주 우선 관찰",
    ...
    return "\n\n".join(
        [
            "📌 종합버젼stock",
            f"주말 종합 리포트 | {start:%Y.%m.%d} ~ {end:%Y.%m.%d}",
            signal_text,
            rs_text,
            stock_text,
            ma_text,
