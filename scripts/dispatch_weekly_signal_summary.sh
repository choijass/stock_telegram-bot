...
WORKFLOW="weekly-signal-summary.yml"
REF="main"
...
STATUS="$(
  curl -sS -o /tmp/stock_telegram_weekly_signal_dispatch_response.txt -w '%{http_code}' \
    -X POST \
...
if [[ "${STATUS}" != "204" ]]; then
  ...
  cat /tmp/stock_telegram_weekly_signal_dispatch_response.txt >&2
  exit 1
fi
